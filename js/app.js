...
async function initAuth() {
  ...
  if (!url || !key) { showLoginScreen('db'); return; }
...
async function loadAll() {
  ...
  DRIVERS = d.data||[];
  VEHICLES = (v.data||[]).map(v=>({...v,truckNumber:v.truck_number,trailerNumber:v.trailer_number,assignedDriverId:v.assigned_driver_id}));
...
async function addVehicle(truckNumber,trailerNumber,assignedDriverId) {
  const rec={id:crypto.randomUUID(),truckNumber,trailerNumber,assignedDriverId:assignedDriverId||null,created_at:new Date().toISOString()};
  VEHICLES.push(rec); await sb.from('vehicles').insert({id:rec.id,truck_number:truckNumber,trailer_number:trailerNumber,assigned_driver_id:assignedDriverId||null,created_at:rec.created_at}); return rec;
}
async function deleteVehicle(id) {
  ...
  TYRE_RECORDS=TYRE_RECORDS.filter(r=>r.vehicleId!==id); DOT_INSPECTIONS=DOT_INSPECTIONS.filter(r=>r.vehicleId!==id); MILEAGE=MILEAGE.filter(r=>r.vehicleId!==id);
  await Promise.all([sb.from('vehicles').delete().eq('id',id),sb.from('maintenance_records').delete().eq('vehicle_id',id),sb.from('brake_tests').delete().eq('vehicle_id',id),sb.from('tyre_records').delete().eq('vehicle_id',id),sb.from('dot_inspections').delete().eq('vehicle_id',id),sb.from('mileage_records').delete().eq('vehicle_id',id)]);
}
...
function getVehicleStatus(vid){
  ...
  const lastBrake=brakes[0],lastTyre=tyres[0],lastDot=dots[0];
  const now=today();
  ...
  const critical=brakeOverdue||hasOOS,warning=brakeDueSoon||tyreOverdue||viciousCircle;
...
function renderDashboard(){
  ...
  html+=`</div></div>`;
  const allRecent=[...MAINTENANCE.map(r=>({date:r.serviceDate,label:`Service – Truck #${VEHICLES.find(v=>v.id===r.vehicleId)?.truckNumber||'?'}`,type:'maint'})),...BRAKE_TESTS.map(r=>({date:r.testDate,label:`Brake ${r.result} – Truck #${VEHICLES.find(v=>v.id===r.vehicleId)?.truckNumber||'?'}`,type:'brake',pass:r.result==='pass'}))].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,6);
  ...
  allRecent.forEach(r=>{const badge=r.type==='brake'?(r.pass?'badge-green':'badge-red'):'badge-blue';html+=`<div class="history-item"><span>${r.label}</span><span class="badge ${badge}">${fmtDate(r.date)}</span></div>`;});
  html+=`</div></div></div>`;
...
function renderVehicles(){
  let html='';
  if(isAdmin()){
    html+=`<div class="card mb-4" style="margin-bottom:20px;max-width:560px"><div class="card-header">🚛 Add Vehicle</div><div class="card-body">
  ...
  VEHICLES.forEach(v=>{
    ...
    html+=`<div class="card" style="cursor:pointer" onclick="navigate('vehicle','${v.id}')"><div class="card-body" style="padding:16px">
      <div class="flex-between mb-4" style="margin-bottom:10px">
        <div><div class="fw-600" style="font-size:15px">Truck #${v.truckNumber}</div><div class="text-sm">Trailer #${v.trailerNumber}</div></div>
        <div style="display:flex;gap:6px;align-items:center">${sb2}${isAdmin()?`<button class="btn btn-ghost btn-sm btn-icon" onclick="event.stopPropagation();doDeleteVehicle('${v.id}','${v.truckNumber}')" title="Delete">🗑</button>`:''}</div>
      </div>
...
async function doAddVehicle(){
  if(!isAdmin()) return;
  const truck=document.getElementById('v-truck').value.trim(),trailer=document.getElementById('v-trailer').value.trim(),driver=document.getElementById('v-driver').value;
  if(!truck||!trailer){showToast('Enter truck and trailer numbers','danger');return;}
  await addVehicle(truck,trailer,driver||null); showToast('Vehicle added!','success'); render();
}
...
function renderVehicleDetail(){
  ...
  const tabs=['maintenance','brakes','tyres','dot','mileage'];
  const tabLabels={maintenance:'🔧 Service',brakes:'🛑 Brakes',tyres:'⭕ Tyres',dot:'📋 DOT',mileage:'📍 Mileage'};
  let html=`<div style="margin-bottom:16px;display:flex;align-items:center;gap:12px">
    ...
    <div><div style="font-size:20px;font-weight:700">Truck #${v.truckNumber}</div><div class="text-sm">Trailer #${v.trailerNumber}${driver?' · Driver: '+driver.name:''}</div></div>
...
let _brakeResult='pass',_dotResult='pass';
...
function renderDrivers(){
  ...
  html+=`<div class="card" style="max-width:480px"><div class="card-header">All Drivers (${DRIVERS.length})</div><div class="card-body">`;
  if(DRIVERS.length===0) html+=`<div class="empty">No drivers added yet</div>`;
  DRIVERS.forEach(d=>{
    ...
    html+=`<div class="history-item" id="driver-row-${d.id}">
      <div id="driver-view-${d.id}" style="flex:1;display:flex;align-items:center;gap:10px"><div><div class="fw-600">${d.name}</div>${vehicles?`<div class="text-sm">${vehicles}</div>`:''}</div></div>
      <div id="driver-edit-${d.id}" style="flex:1;display:none;gap:8px;align-items:center"><input type="text" value="${d.name}" id="dedit-${d.id}" style="flex:1"/><button class="btn btn-success btn-sm" onclick="doUpdateDriver('${d.id}')">Save</button><button class="btn btn-ghost btn-sm" onclick="cancelEditDriver('${d.id}')">Cancel</button></div>
      ${isAdmin()?`<div style="display:flex;gap:6px;flex-shrink:0" id="driver-btns-${d.id}"><button class="btn btn-ghost btn-sm" onclick="startEditDriver('${d.id}')">✏ Edit</button><button class="btn btn-ghost btn-sm btn-icon" onclick="doDeleteDriver('${d.id}','${d.name.replace(/'/g,"\\'")}')">🗑</button></div>`:''}
    </div>`;
  });
  html+=`</div></div>`;
  return html;
...
function renderReports(){
  ...
  <div class="card"><div class="card-header">Per-Vehicle Summary</div><div class="card-body" style="padding:0"><div class="table-wrap"><table>
    ...
    <tbody>${VEHICLES.length===0?`<tr><td colspan="4" class="empty">No vehicles</td></tr>`:VEHICLES.map(v=>{const s=getVehicleStatus(v.id);return`<tr style="cursor:pointer" onclick="navigate('vehicle','${v.id}')"><td><strong>Truck #${v.truckNumber}</strong></td><td>${s.lastBrake?fmtDate(s.lastBrake.testDate):'—'}</td><td>${s.lastTyre?fmtDate(s.lastTyre.photoDate):'—'}</td><td><span class="badge ${s.critical?'badge-red':s.warning?'badge-yellow':'badge-green'}">${s.critical?'Critical':s.warning?'Warning':'OK'}</span></td></tr>`;}).join('')}</tbody>
  </table></div></div></div>
  </div>`;
  return html;
}
