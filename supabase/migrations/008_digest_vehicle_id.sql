-- ================================================================
-- Migration 008 — expose vehicle_id from dispatcher_digest_data()
--
-- The morning digest applied fleet-wide thresholds (brake 35 / service 60 /
-- tyre 14) to every truck. To honour per-vehicle overrides in
-- reminder_schedules, the Edge Function needs the vehicle id to join on.
-- Only the added `vehicle_id` column differs from migration 007 — the query
-- body is otherwise unchanged.
--
-- Adding a column changes the return signature, so the old function must be
-- dropped first (CREATE OR REPLACE cannot alter RETURNS TABLE).
-- ================================================================
DROP FUNCTION IF EXISTS dispatcher_digest_data();

CREATE FUNCTION dispatcher_digest_data()
RETURNS TABLE (
